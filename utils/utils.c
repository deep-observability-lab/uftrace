...
int make_cpulist_str(uint32_t *mask, int nr_cpus, char *buf, int bufsize)
{
	int i, run_start = -1;
	int len = 0;
	bool first = true;
	...
	for (i = 0; i <= nr_cpus; i++) {
		bool bit = (i < nr_cpus) && (mask[i / 32] & (1U << (i % 32)));
...
int make_cpumask_str(uint32_t *mask, int nr_cpus, char *buf, int bufsize)
{
	...
	for (i = ngroups - 1; i >= 0; i--) {
		...
		n = snprintf(buf + len, bufsize - len, "%s%08x", i == ngroups - 1 ? "" : " ",
			     mask[i]);
...
TEST_CASE(utils_make_cpulist_str)
{
	...
	pr_dbg("full string with ranges and singletons\n");
	/* CPUs 0-32, 34-40, 42-45, 47, 50 - reused by truncation tests below */
	mask[0] = 0xFFFFFFFFU;
	mask[1] = 1U | (0x7FU << 2) | (0xFU << 10) | (1U << 15) | (1U << 18);
	make_cpulist_str(mask, 64, buf, sizeof(buf));
	TEST_STREQ(buf, "0-32/34-40/42-45/47/50");

	pr_dbg("truncation backtracks the last entry to fit /...\n");
	/*
	 * Same mask with bufsize=21: writes "0-32/34-40/42-45/47" then "/50"
	 * doesn't fit; backtrack uses '/' at index 16 and overwrites the last
	 * "/47" entry with "/..."
	 */
